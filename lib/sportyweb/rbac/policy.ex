defmodule Sportyweb.RBAC.Policy do
  ...
  defp error_redirect(action, view, params) when action in [:new, :show], do: get_redirect_path_new_show(view, params)
  ...
  defp get_redirect_path_new_show(:GroupLive, %{"department_id" => department_id}), do: ~p"/departments/#{department_id}/groups/"
  defp get_redirect_path_new_show(:EquipmentLive, %{"venue_id" => venue_id}), do: ~p"/venues/#{venue_id}/equipment/"
  ...
  defp get_button_config(user, view, params) do
    [index, edit, show, new] = [:index, :edit, :show, :new] |> Enum.map(&(permit?(user, &1, view, params)))
    %ButtonConfig{id: 1, index: index, edit: edit, show: show, new: new}
  end

  # <--- Policy streams and assigns ---> #
  defp add_streams(socket, :ClubLive, :index), do: Phoenix.LiveView.attach_hook(socket, :add_stream_club, :handle_params, &add_stream_club/3)
  defp add_streams(socket, _view, _action), do: socket

  defp add_assigns(socket, _view, _action), do: Phoenix.LiveView.attach_hook(socket, :add_assign_button_config, :handle_params, &add_assign_button_config/3)

  defp add_stream_club(_params, _url, socket) do
    socket = socket.assigns.current_user
