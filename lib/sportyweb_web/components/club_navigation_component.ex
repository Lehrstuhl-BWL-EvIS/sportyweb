defmodule SportywebWeb.ClubNavigationComponent do
    ...
    <nav class="sticky top-10">
      <div class="space-y-1">
        <.link navigate={~p"/clubs/#{@club}"}
          class={[@classes_menu_item, (if @club_navigation_current_item == :dashboard, do: @classes_menu_item_active)]}>
          <.icon name="hero-rocket-launch" class={@classes_icon} />
          <span class="truncate">Dashboard</span>
        </.link>

        <.link navigate={~p"/clubs/#{@club}/events"}
          class={[@classes_menu_item, (if @club_navigation_current_item == :calendar, do: @classes_menu_item_active)]}>
          <.icon name="hero-calendar" class={@classes_icon} />
          <span class="truncate">Kalender</span>
        </.link>

        <.link navigate={~p"/clubs/#{@club}/departments"}
          class={[@classes_menu_item, (if @club_navigation_current_item == :structure, do: @classes_menu_item_active)]}>
        ...
        <.link navigate={~p"/clubs/#{@club}/contacts"}
          class={[@classes_menu_item, (if @club_navigation_current_item == :contacts, do: @classes_menu_item_active)]}>
        ...
        <.link navigate={~p"/clubs/#{@club}/venues"}
          class={[@classes_menu_item, (if @club_navigation_current_item == :assets, do: @classes_menu_item_active)]}>
          <.icon name="hero-building-office-2" class={@classes_icon} />
          <span class="truncate">Standorte & Equipment</span>
        </.link>

        <.link phx-target={@myself} phx-click="toggle_submenu" phx-value-item={:finances}
          class={[@classes_menu_item, (if @club_navigation_current_item == :finances, do: @classes_menu_item_active)]}>
        ...
        <ul class={["mb-1 px-2", (if !@show_submenu_finances, do: "hidden")]}>
          <li>
            <.link navigate={~p"/clubs/#{@club}/transactions"}
              class={[
          ...
          <li>
            <.link navigate={~p"/clubs/#{@club}/subsidies"}
              class={[
                @classes_menu_item, @classes_submenu_item,
        ...
        <.link navigate={~p"/clubs/#{@club}/roles"}
          class={[@classes_menu_item, (if @club_navigation_current_item == :authorization, do: @classes_menu_item_active)]}>
  ...
  def update(assigns, socket) do
    show_submenu_finances = assigns.club_navigation_current_item == :transactions ||
    ...
    {:ok,
     socket
     |> assign(assigns)
     |> assign(:classes_menu_item, "text-zinc-700 hover:bg-zinc-50 group flex items-center px-3 py-2 text-base lg:text-sm font-medium rounded-md")
