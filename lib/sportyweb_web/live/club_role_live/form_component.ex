defmodule SportywebWeb.ClubRoleLive.FormComponent do
  ...
  def update(%{club_role: club_role} = assigns, socket) do
    changeset = AccessControl.change_club_role(club_role)
  ...
  def handle_event("validate", %{"club_role" => club_role_params}, socket) do
    changeset =
      ...
      |> AccessControl.change_club_role(club_role_params)
  ...
  defp save_club_role(socket, :edit, club_role_params) do
    case AccessControl.update_club_role(socket.assigns.club_role, club_role_params) do
      {:ok, _club_role} ->
        {:noreply,
         socket
  ...
  defp save_club_role(socket, :new, club_role_params) do
    case AccessControl.create_club_role(club_role_params) do
      {:ok, _club_role} ->
        {:noreply,
         socket
